...
```{r nonfilt-income-plot, fig.width = 10}
...
```{r full}
full_int_new_2 <- lm(hhincome ~ sex + age + statebrn + marstat + edyrs + occtype + usdur1 + usdoc1 + uscity + age*edyrs + sex*usdur1+ statebrn*edyrs+statebrn*usdurl, data=caldata)
...
```{r}
x0 <- data.frame(sex = "M" , age = 0, edyrs = 6, usdur1 = 0, usdurl = 0, statebrn="Bajío")
...
```{r}
x1 <- data.frame(sex = "F" , age = 0, edyrs = 6, usdur1 = 0, usdurl = 0, statebrn="Bajío")
...
```{r}
x3 <- data.frame(sex = "M" , age = 0, edyrs = 6, usdur1 = 0, usdurl = 0, statebrn="Central Mexico")
...
```{r}
x4 <- data.frame(sex = "M" , age = 0, edyrs = 6, usdur1 = 0, usdurl = 0, statebrn="Northern Mexico")
...
```{r}
x5 <- data.frame(sex = "M" , age = 0, edyrs = 6, usdur1 = 0, usdurl = 0, statebrn="South East Mexico")
...
```{r}
x5 <- data.frame(sex = "M" , age = 0, edyrs = 6, usdur1 = 0, usdurl = 0, statebrn="Pacific Coast")
predict.lm(reduced_int_new_2, x5, interval = "confidence")
```
...
Using backward selection based on AIC, we narrowed down to 6 variables: sex, edyrs, usdurl, statebrn, occtype and age.


### 2.3 Interactions

To find potential interactions between the 6 variables, we used nested-F test for each of the possible interactions:

After inital backwards selection, we will explore the possible interactions between remaining variables: `edyrs`, `usdurl`, `sex`, `statebrn`, `occtype`, and `age`, to determine if any are significant.

```{r int-model}
int_model_1 <- lm(hhincome ~ sex + age + statebrn + edyrs +occtype+ usdurl + edyrs*usdurl, data = caldata)
anova(reduced, int_model_1, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.7366925
```
...
```{r int-model2}
int_model_2 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + sex*usdurl, data = caldata)
anova(reduced, int_model_2, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3) # p = 0.0303039
```
...
```{r int-model3}
int_model_3 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + age*usdurl, data = caldata)
anova(reduced, int_model_3, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.1936359
```
...
```{r int-model4}
int_model_4 <- lm(hhincome ~ sex + age +statebrn+ edyrs+occtype + usdurl + sex*edyrs, data = caldata)
anova(reduced, int_model_4, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.5924992
```
...
```{r int-model5}
int_model_5 <- lm(hhincome ~ sex + age + statebrn+ edyrs+occtype + usdurl + age*edyrs, data = caldata)
...
```{r int-model 6}
int_model_6 <- lm(hhincome ~ sex + age + statebrn + edyrs+occtype + usdurl + age*sex, data = caldata)
anova(reduced, int_model_6, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.2073624
```
...
```{r int-model 7}
int_model_7 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + statebrn*sex, data = caldata)
anova(reduced, int_model_7, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.3796077
```
...
```{r int-model 8}
int_model_8 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + statebrn*age, data = caldata)
anova(reduced, int_model_8, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.213341
```
...
```{r int-model 9}
int_model_9 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + statebrn*edyrs, data = caldata)
anova(reduced, int_model_9, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.0002466
```
...
```{r int-model 10}
int_model_10 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + statebrn*usdurl, data = caldata)
anova(reduced, int_model_10, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.0012
```
...
```{r int-model 11}
int_model_11 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + occtype*sex, data = caldata)
anova(reduced, int_model_11, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.0012
```
Then, we tested the interaction between `occtype` and `sex`. The p-value for this test was 0.0012 < 0.05, therefore we will include it in the model.

```{r int-model 12}
int_model_12 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + occtype*age, data = caldata)
anova(reduced, int_model_12, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.0012
```
Then, we tested the interaction between `occtype` and `age`. The p-value for this test was 0.0012 < 0.05, therefore we will include it in the model.

```{r int-model 13}
int_model_13 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + occtype*statebrn, data = caldata)
anova(reduced, int_model_13, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.0012
```
Then, we tested the interaction between `occtype` and `statebrn`. The p-value for this test was 0.0012 < 0.05, therefore we will include it in the model.

```{r int-model 14}
int_model_14 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + occtype*edyrs, data = caldata)
anova(reduced, int_model_14, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.0012
```
Then, we tested the interaction between `occtype` and `edyrs`. The p-value for this test was 0.0012 < 0.05, therefore we will include it in the model.

```{r int-model 15}
int_model_15 <- lm(hhincome ~ sex + age + statebrn+edyrs+occtype + usdurl + occtype*usdurl, data = caldata)
anova(reduced, int_model_15, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.0012
```
Then, we tested the interaction between `occtype` and `usdurl`. The p-value for this test was 0.0012 < 0.05, therefore we will include it in the model.
...
```{r final-model}
reduced_int <- lm(hhincome ~ sex + age + edyrs + usdurl + age*edyrs + sex*usdurl + statebrn*edyrs+statebrn*usdurl, data=caldata)
...
```{r}
full_int <- lm(hhincome ~ sex + age + statebrn + marstat + edyrs + occtype + usdur1 + usdurl + usdoc1 + uscity + sex*usdurl + age*edyrs +statebrn*edyrs+statebrn*usdurl , data=caldata)
...
```{r augment}
mig_output <- augment(reduced_int_new_2) %>%
...
```{r std-resid-predictor-plots, fig.width = 16, fig.height = 8}
...
cowplot::plot_grid(ageresidplot, edyrsresidplot, usdurlresidplot, sexresidplot, statebrnresidplot)
```
...
glance(reduced_int_new_2)$sigma
...
```{r int-model-2}
int_model_1 <- lm(hhincome ~ sex + age + statebrn + edyrs + usdurl + edyrs*usdurl, data = caldata)
anova(reduced, int_model_1, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.7366925
```
First, we tested the interaction between `edyrs` and `usdurl`. The p-value for this test was 0.7366925 > 0.05, therefore we will not include it in the model.
...
```{r int-model2-2}
int_model_2 <- lm(hhincome ~ sex + age + statebrn+edyrs + usdurl + sex*usdurl, data = caldata)
anova(reduced, int_model_2, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3) # p = 0.0303039
```
Then, we tested the interaction between `sex` and `usdurl`. The p-value for this test was 0.0303039 < 0.05, therefore we will include it in the model.
...
```{r int-model3-2}
int_model_3 <- lm(hhincome ~ sex + age + statebrn+edyrs + usdurl + age*usdurl, data = caldata)
anova(reduced, int_model_3, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.1936359
```
Then, we tested the interaction between `age` and `usdurl`. The p-value for this test was 0.1936359 > 0.05, therefore we will not include it in the model.
...
```{r int-model4-2}
int_model_4 <- lm(hhincome ~ sex + age +statebrn+ edyrs + usdurl + sex*edyrs, data = caldata)
anova(reduced, int_model_4, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.5924992
```
Then, we tested the interaction between `sex` and `edyrs`. The p-value for this test was 0.5024992 > 0.05, therefore we will not include it in the model.
...
```{r int-model5-2}
int_model_5 <- lm(hhincome ~ sex + age + statebrn+ edyrs + usdurl + age*edyrs, data = caldata)
anova(reduced, int_model_5, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0
```
Then, we tested the interaction between `age` and `edyrs`. The p-value for this test was 0 < 0.05, therefore we will include it in the model.
...
```{r int-model 6-2}
int_model_6 <- lm(hhincome ~ sex + age + statebrn + edyrs + usdurl + age*sex, data = caldata)
anova(reduced, int_model_6, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.2073624
```
Then, we tested the interaction between `age` and `sex`. The p-value for this test was 0.2073624 > 0.05, therefore we will not include it in the model.
...
```{r int-model 7-2}
int_model_7 <- lm(hhincome ~ sex + age + statebrn+edyrs + usdurl + statebrn*sex, data = caldata)
anova(reduced, int_model_7, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.3796077
```
Then, we tested the interaction between `statebrn` and `sex`. The p-value for this test was  0.3796077> 0.05, therefore we will not include it in the model.
...
```{r int-model 8-2}
int_model_8 <- lm(hhincome ~ sex + age + statebrn+edyrs + usdurl + statebrn*age, data = caldata)
anova(reduced, int_model_8, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.213341
```
Then, we tested the interaction between `statebrn` and `age`. The p-value for this test was  0.213341> 0.05, therefore we will not include it in the model.
...
```{r int-model 9-2}
int_model_9 <- lm(hhincome ~ sex + age + statebrn+edyrs + usdurl + statebrn*edyrs, data = caldata)
anova(reduced, int_model_9, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.0002466
```
Then, we tested the interaction between `statebrn` and `edyrs`. The p-value for this test was  0.0002466< 0.05, therefore we will  include it in the model.
...
```{r int-model 10-2}
int_model_10 <- lm(hhincome ~ sex + age + statebrn+edyrs + usdurl + statebrn*usdurl, data = caldata)
anova(reduced, int_model_10, test = "Chisq") %>% 
  kable(format = "markdown", digits = 3)  # p = 0.0012
```
Then, we tested the interaction between `statebrn` and `usdurl`. The p-value for this test was 0.0012 < 0.05, therefore we will include it in the model.
...
```{r vif}
tidy(vif(reduced_int_new_2))
```

```{r}
summary(reduced_int_new_2)$adj.r.squared
...
region <- data.frame("Region" = c( "Central Mexico", "Northern Mexico","Bajío", "Pacific Coast", "South East Mexico" ), "pred_hhincome" = c( 1611.19, 1056.73,893.32,814.86,788.32 ))
